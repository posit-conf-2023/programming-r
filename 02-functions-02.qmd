...
Changing fill color: *as is*, there is no easy way to specify `"steelblue"`
...
```r
# extra args `...` passed to `geom_histogram()`
histogram <- function(df, var, ..., binwidth = NULL) {
  df |>
    ggplot(aes(x = {{var}})) +
    geom_histogram(binwidth = binwidth, ...)
...
Adapt the `histogram()` function to include a title that describes:
